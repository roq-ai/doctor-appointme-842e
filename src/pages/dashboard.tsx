...
import { getSystemAdministrators } from 'apiSdk/system-administrators';
...
function DashboardPage() {
  ...
  const systemAdministratorStat = useSWR<PaginatedInterface<SystemAdministratorInterface>>(
    () => `/system-administrators`,
    () => getSystemAdministrators({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: systemAdministratorStat?.data?.totalCount,
      isLoading: systemAdministratorStat.isLoading,
